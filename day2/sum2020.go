...
func FormatData(input []string) []Policy {
	n := make([]Policy, len(input))
	for i, v := range input {
		arr := strings.Split(v, " ")
		lower, upper := ParsePolicyRange(arr[0])
		letter := strings.TrimSuffix(arr[1], ":")
		password := arr[2]
		n[i] = Policy{lower, upper, letter, password}
	}
	return n
}

func ParsePolicyRange(str string) (int, int) {
	var l string
	var u string
	found := false
	for _, v := range str {
	...
	il, err1 := strconv.Atoi(l)
	iu, err2 := strconv.Atoi(u)
	PanicOnError(err1)
	PanicOnError(err2)
	return il, iu
}
